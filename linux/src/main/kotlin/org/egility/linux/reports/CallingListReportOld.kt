 ...
 */
class CallingListReportOld(val idCompetition: Int, val idAgilityClass: Int, outfile: String, copies: Int, val formal: Boolean=false, outStream: OutputStream? = null) : Report(outfile, copies, outStream) {
...
//        net.sf.jasperreports.default.pdf.encoding=UTF-8
        ...
        val runningOrderColumn = col.column("R/O", "runningOrder", type.integerType()).setWidth(12)
        val runningOrderColumn2 = col.column("R/O", "runningOrder", type.integerType()).setWidth(12)
        val tickColumn = col.column("Tick", "blank", type.stringType()).setWidth(14)
        ...
        reportBuilder
                .pageHeader(
                        cmp.verticalList(
                                ...
                                cmp.text("CALLING SHEET (Emergency Score Sheet)")
                                        .setStyle(titleStyle)
                                        .setHorizontalTextAlignment(HorizontalTextAlignment.CENTER)
                        )
                )
    ...
    companion object : ReportCompanion {
        ...
        fun generate(idCompetition: Int = 0, idAgilityClass: Int = 0, copies: Int = 1, formal: Boolean=false, pdf: Boolean = false, regenerate: Boolean = true): String {
            ...
            if (outFile.isEmpty() || regenerate || !File(outFile).cached()) {
                CallingListReportOld(idCompetition, idAgilityClass, outFile, copies, formal)
            }
            return outFile
        }
