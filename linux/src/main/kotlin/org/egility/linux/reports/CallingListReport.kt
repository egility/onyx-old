 ...
 */
class CallingListReport(val idCompetition: Int, val idAgilityClass: Int, outfile: String, copies: Int, val formal: Boolean=false, outStream: OutputStream? = null) : Report(outfile, copies, outStream) {
...
//        net.sf.jasperreports.default.pdf.encoding=UTF-8
        ...
        val runningOrderColumn = col.column("R/O", "runningOrder", type.integerType()).setWidth(16)
        val runningOrderColumn2 = col.column("R/O", "runningOrder", type.integerType()).setWidth(12)
        val tickColumn = col.column("Tick", "blank", type.stringType()).setWidth(18)
        val nameColumn = col.column("Name", "name", type.stringType()).setHorizontalTextAlignment(HorizontalTextAlignment.LEFT)
        val gradeColumn = col.column("Gr", "grade", type.stringType()).setWidth(10).setHorizontalTextAlignment(HorizontalTextAlignment.CENTER)
        val abcColumn = col.column("ABC", "abc", type.stringType()).setWidth(14).setHorizontalTextAlignment(HorizontalTextAlignment.CENTER)
        val heightColumn = col.column("Height", "height", type.stringType()).setWidth(18).setHorizontalTextAlignment(HorizontalTextAlignment.CENTER)
        ...
        val faultsColumn = col.column("Faults", "blank", type.stringType()).setWidth(36)
        val timeColumn = col.column("Time", "blank", type.stringType()).setWidth(24)
        ...
        reportBuilder
            ...
            .setPageColumnSpace(10)
                .columns(runningOrderColumn, tickColumn, nameColumn)
//                .columns(runningOrderColumn, tickColumn, nameColumn, runningOrderColumn2, faultsColumn, timeColumn, if (competition.isFab) abcColumn else gradeColumn, heightColumn)
    ...
    companion object : ReportCompanion {
        ...
        override val keyword = Reports.CALLING_SHEET
        ...
        fun generate(idCompetition: Int = 0, idAgilityClass: Int = 0, copies: Int = 1, formal: Boolean=false, pdf: Boolean = false, regenerate: Boolean = true): String {
            ...
            if (outFile.isEmpty() || regenerate || !File(outFile).cached()) {
                CallingListReport(idCompetition, idAgilityClass, outFile, copies, formal)
            }
            return outFile
        }
